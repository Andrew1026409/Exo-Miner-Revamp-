...
void SplatmapVert(inout appdata_full v, out Input data)
{
    ...
    #if !defined(INTERRA_TERRAIN) || defined(TRIPLANAR)
...
//==============================================================================================
#ifdef DIFFUSE
    void SplatmapMix(Input IN, half4 defaultAlpha, out half weight, out fixed4 mixedDiffuse, out fixed3 mixedNormal)
#else
...
#endif
{
    ...
    mixedNormal = float3( 0 ,0, 1);
        ...
        #if defined(_OBJECT_PARALLAX) && !defined(DIFFUSE)
    ...
    #if !defined(INTERRA_TERRAIN) || defined(TRIPLANAR) || defined(_TERRAIN_TRIPLANAR_ONE)
    ...
    //================================================================================
    #ifndef INTERRA_TERRAIN
        float2 uv = _WorldMapping ? (IN.worldPos.xz - _TerrainPosition.xz) : IN.worldPos.xz;

        #if !defined(_OBJECT_TRIPLANAR)
            _SteepDistortion = wNormal.y > 0.5 ? 0 : (1 - wNormal.y) * _SteepDistortion;
            UvSplatDistort(uvSplat, uv, objectAlbedo * _SteepDistortion);
        #else
            UvSplat(uvSplat, uv);
        #endif
    #else
        float2 uv = _WorldMapping ? (IN.worldPos.xz / _TerrainSizeXZPosY) : IN.tc.xy;
        uvSplat[0] = TRANSFORM_TEX(uv, _Splat0);
        uvSplat[1] = TRANSFORM_TEX(uv, _Splat1);
        #if !defined(_LAYERS_TWO)
            uvSplat[2] = TRANSFORM_TEX(uv, _Splat2);
            uvSplat[3] = TRANSFORM_TEX(uv, _Splat3);
        #endif
    #endif   
    ...
    //-------------------- PARALLAX OFFSET -------------------------                   
    #if defined(_TERRAIN_PARALLAX) && !defined(DIFFUSE)
    if(_Terrain_Parallax)
    { 
    ...
    //========================================================================================
    #if defined(_TERRAIN_BLEND_HEIGHT) && !defined(_LAYERS_ONE) && !defined(DIFFUSE) && !defined(TERRAIN_SPLAT_ADDPASS)
    ...
    //========================================================================================
        ...
        #if defined(TRIPLANAR) && !defined(TERRAIN_BASE_PASS)
            half3 normalFront;
            half3 normalSide;
    ...
    { 
       ...
       float heightSum = 0.5;
            ...
            {
                #if defined(DIFFUSE)
                    heightSum = HeightSum(splat, splat_control);
                #else
                    heightSum = HeightSum(mask, splat_control);
                #endif    
            }
        #endif
        ...
        mixedDiffuse.a = lerp(mixedDiffuse.a, trackValues.x, track);
            ...
            float4 heights[4];
             heights[0] = tex2Dlod(_InTerra_TrackTexture, float4(trackUV + float2(0.0,  normalsOffset), 0, 0));
             heights[1] = tex2Dlod(_InTerra_TrackTexture, float4(trackUV + float2( normalsOffset, 0.0), 0, 0));
             heights[2] = tex2Dlod(_InTerra_TrackTexture, float4(trackUV + float2(-normalsOffset, 0.0), 0, 0));
             heights[3] = tex2Dlod(_InTerra_TrackTexture, float4(trackUV + float2(0.0, -normalsOffset), 0, 0));
    ...
    //=======================================================================================
        ...
        mixedNormal = lerp(mixedNormal, mainNorm, heightIntersect.r);
            ...
            mixedNormal = mixedNormal + (half(dot(splat_control.xy, half2(splat[0].r, splat[1].r))) * 0.00001f); //this is just a workaround for some Unity bug to prevent missing _Splat0 sampler
    #endif
    //=========================================================================================
