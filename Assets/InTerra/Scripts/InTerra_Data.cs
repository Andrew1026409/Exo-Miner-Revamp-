...
namespace InTerra
{
	public static class InTerra_Data
	{
		...
		{
			if (UpdateScript.GlobalData == null)
			{
				#if UNITY_EDITOR
				...
				UpdateScript.GlobalData = Resources.Load<InTerra_GlobalData>("InTerra_GlobalData");
			}
			return UpdateScript.GlobalData;
		}
		...
		public static void TracksUpdate()
		{
			...
			if (Updater != null)
			{
				...
				{
					if (TrackCamera.enabled == false) TrackCamera.enabled = true;

					#if UNITY_EDITOR
						if (UpdateScript.TrackTexture.height != GetGlobalData().trackTextureSize) CreateTrackRenderTexture();
					...
					Updater.transform.position = new Vector3(tcPos.x,  - 10.0f - UpdateScript.TrackDepthIndex, tcPos.z);
		...
		public static void CheckAndUpdate()
		{
			...
			if (terrains.Length > 0 || (meshTerrainList != null && meshTerrainList.Count > 0))
			{
                ...
                {
					if (Updater.TryGetComponent<Camera>(out Camera cam))
					{
						cam.enabled = false;
					}
				}
		...
		{
			return UpdateScript.TracksFading;
		}

		public static void SetTracksFading(bool enable)
		{
			UpdateScript.TracksFading = enable;
		}

		public static void SetTracksFadingTime(float time)
		{
			UpdateScript.TracksFadingTime = time;
		}
